...
**Status:** PARTIAL (quota-limited). Real `ionq_forte` H₂ data captured: first job + 10-run energy ensemble (mean −0.930 Ha, σ≈0.016) + shot-ladder rungs at 100 & 1000 shots (sharpening confirmed: −0.856 → −0.926 Ha toward −1.1168). **Blocked by IonQ `QuotaExhaustedError` at ~$9.5k actual spend** (real quota ≪ assumed 50k; quota-rejected jobs cost $0): the 10k-shot ladder rung and Batch 3 (hardware 1-RDM *uncertainty cloud*) could not run — the trajectory cloud falls back to the documented sim ensemble (F5). **LiH hardware proven economically infeasible** (UCCSD ≈ 6,700 2q-gates → ~$8k/circuit; one energy eval ≈ $1.3M) → LiH stays sim-only alongside HeH⁺. Real Forte per-circuit cost ran ~2.3× the `/jobs/estimate` quote. Bundles + Pages regenerated from the real run logs. Code (shots→precision control fix, `ionq_forte`-default run scripts, hardware-RDM ensemble path) landed and tested (169 passed). Remaining (RDM cloud + 10k rung) gated on additional IonQ quota.
